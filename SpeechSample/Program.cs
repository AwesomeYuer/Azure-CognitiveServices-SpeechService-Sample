...
Helper.RegisterSpeechConfig(subscriptionKey ?? "");
var speechConfig = Helper.SpeechConfig;
//https://docs.microsoft.com/en-us/azure/cognitive-services/speech-service/language-support
speechConfig.SpeechSynthesisLanguage = locale;
speechConfig.OutputFormat = OutputFormat.Detailed;

await GenerateVoiceOutput.MainTestAsync();
Console.ReadLine();
...
foreach (var voice in voices)
{
    ...
    }
    {
        ...
        using var speechSynthesizer = new SpeechSynthesizer(speechConfig, output);
        using var speechSynthesisResult = await speechSynthesizer.SpeakTextAsync(text);
        Console.WriteLine($"Result: {speechSynthesisResult.Reason}");
    }
    {
        using var streamSynthesizer = new SpeechSynthesizer(speechConfig, null);
    ...
    {
        using var speechRecognizer = new SpeechRecognizer(speechConfig, autoDetectSourceLanguageConfig, input);
